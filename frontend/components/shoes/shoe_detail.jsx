...
class ShoeDetail extends React.Component {
  constructor(props) {
    ...
    let shoeId = this.props.location.pathname.split("/")[2];
  ...
  componentDidMount() {
    ...
    // Object.values(this.props.follows).forEach(
    //     follow => {if (parseInt(follow.shoe_id) === parseInt(this.state.shoeId)){
  ...
  render() {
    ...
    let sales = this.props.sales[this.state.shoeId]
      ? Object.values(this.props.sales[this.state.shoeId])
      : [];
    let orders = isEmpty(this.props.orders)
      ? [{ price: 0, type: "buy" }]
    ...
    return (
      <section className="shoe-detail-container">
        ...
        <span className="shoe-detail-misc">
          <span className="shoe-detail-condition">
            Condition:
            <p style={{ color: "green" }}>New</p>
          </span>
          |
          <span className="shoe-detail-ticker">
            Ticker:
        ...
        <div className="shoe-detail-img-container">
          <img src={shoe.photoUrl} alt="" className="shoe-detail-img" />
